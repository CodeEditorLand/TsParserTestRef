...
module Errors {
	...
	declare function foo16(a2: {
		...
		(x: { (a: number): number; (a?: number): number }): number[];
		(x: { (a: boolean): boolean; (a?: boolean): boolean }): boolean[];
	}): typeof a2;
	...
	var r2arg =
		<T extends Base, U extends Derived, V extends Derived2>(
			x: (arg: T) => U,
		) =>
		(r: T) =>
			<V>null;
	...
	var r3arg =
		<T extends Base, U extends Derived>(
			x: (arg: T) => U,
			y: (arg2: { foo: number }) => U,
		) =>
		(r: T) =>
			<U>null;
	var r3arg2 =
		(x: (arg: Base) => Derived, y: (arg2: Base) => Derived) => (r: Base) =>
			<Derived>null;
	...
	var r5arg2 = (x: { foo: string }, y: { foo: string; bar: string }) =>
		<Base>null;
	...
	var r6arg2 = <T extends Array<Derived2>>(x: Array<Base>, y: Array<Base>) =>
		<T>null;
