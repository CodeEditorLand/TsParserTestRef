...
module TypeScript {
	...
	export function pushAssignScope(
		scope: SymbolScope,
	...
	export var instanceFilter = new ScopeSearchFilter(
		instanceCompare,
		instanceFilterStop,
	);

	export function preAssignModuleScopes(
		ast: AST,
		context: AssignScopeContext,
	) {
		...
		memberScope = new SymbolTableScope(
			mod.members,
			mod.ambientMembers,
			mod.enclosedTypes,
			mod.ambientEnclosedTypes,
			mod.symbol,
		);
		...
		if (mod.symbol) {
			context.typeFlow.addLocalsFromScope(
				mod.containedScope,
				mod.symbol,
				moduleDecl.vars,
				mod.members.privateMembers,
				true,
			);
		}
	}

	export function preAssignClassScopes(
		ast: AST,
		context: AssignScopeContext,
	) {
		...
		if (classType) {
			...
			memberScope = <SymbolTableScope>(
				context.typeFlow.checker.scopeOf(classType)
			);
			...
			memberScope = <SymbolTableScope>(
				context.typeFlow.checker.scopeOf(instanceType)
			);
	...
	export function preAssignInterfaceScopes(
		ast: AST,
		context: AssignScopeContext,
	) {
		...
		memberScope = <SymbolTableScope>(
			context.typeFlow.checker.scopeOf(interfaceType)
		);
	...
	export function preAssignWithScopes(ast: AST, context: AssignScopeContext) {
		...
		var members = new ScopedMembers(
			new DualStringHashTable(
				new StringHashTable(),
				new StringHashTable(),
			),
		);
		var ambientMembers = new ScopedMembers(
			new DualStringHashTable(
				new StringHashTable(),
				new StringHashTable(),
			),
		...
		var withSymbol = new WithSymbol(
			withStmt.minChar,
			context.typeFlow.checker.locationInfo.unitIndex,
			withType,
		);
		...
		var withScope = new TypeScript.SymbolScopeBuilder(
			withType.members,
			withType.ambientMembers,
			null,
			null,
	...
	export function preAssignFuncDeclScopes(
		ast: AST,
		context: AssignScopeContext,
	) {
		...
		var parentScope = isInnerStatic
			? context.scopeChain.fnc.type.memberScope
			: context.scopeChain.scope;
		...
		if (
			...
			if (
				!(instType.typeFlags & TypeFlags.IsClass) &&
				!hasFlag(funcDecl.fncFlags, FncFlags.ClassMethod)
			) {
				...
				if (
					context.scopeChain.previous.scope.container &&
		...
		if (
			...
			if (isStatic) {
				...
				// compatibility tests
				if (
					outerFnc.type.members == null &&
					container.getType().memberScope
				) {
					outerFnc.type.members = (<SymbolScopeBuilder>(
						(<TypeSymbol>container).type.memberScope
					)).valueMembers;
				}
				funcScope = context.scopeChain.fnc.type.memberScope;
				outerFnc.innerStaticFuncs[outerFnc.innerStaticFuncs.length] =
					funcDecl;
			} else {
				if (
					!funcDecl.isConstructor &&
			...
			context.typeFlow.checker.createFunctionSignature(
				funcDecl,
				container,
				funcScope,
				fgSym,
			...
			if (
				(!funcDecl.accessorSymbol &&
					funcDecl.fncFlags & FncFlags.ClassMethod &&
					container &&
					(!fgSym || fgSym.declAST.nodeType != NodeType.FuncDecl) &&
					funcDecl.isAccessor()) ||
				(fgSym && fgSym.isAccessor())
			) {
				funcDecl.accessorSymbol =
					context.typeFlow.checker.createAccessorSymbol(
						funcDecl,
						fgSym,
						container.getType(),
						funcDecl.isMethod() && isStatic,
						true,
						funcScope,
						container,
					);
		...
		var funcMembers = new ScopedMembers(
			new DualStringHashTable(funcTable, new StringHashTable()),
		);
		var ambientFuncTable = new StringHashTable();
		var ambientFuncMembers = new ScopedMembers(
			new DualStringHashTable(ambientFuncTable, new StringHashTable()),
		);
		var funcStaticTable = new StringHashTable();
		var funcStaticMembers = new ScopedMembers(
			new DualStringHashTable(funcStaticTable, new StringHashTable()),
		);
		var ambientFuncStaticTable = new StringHashTable();
		var ambientFuncStaticMembers = new ScopedMembers(
			new DualStringHashTable(
				ambientFuncStaticTable,
				new StringHashTable(),
			),
		);
		...
		var locals = new SymbolScopeBuilder(
			funcMembers,
			ambientFuncMembers,
			null,
			null,
		...
		var statics = new SymbolScopeBuilder(
			funcStaticMembers,
			ambientFuncStaticMembers,
			null,
			null,
			parentScope,
			null,
		...
		if (!funcDecl.isSpecialFn()) {
			...
			if (signature.parameters) {
				...
				for (var i = 0; i < len; i++) {
					...
					context.typeFlow.checker.resolveTypeLink(
						locals,
						paramSym.parameter.typeLink,
						true,
					);
				}
			}
	...
	export function preAssignCatchScopes(
		ast: AST,
		context: AssignScopeContext,
	) {
		var catchBlock = <Catch>ast;
		if (catchBlock.param) {
			var catchTable = new ScopedMembers(
				new DualStringHashTable(
					new StringHashTable(),
					new StringHashTable(),
				),
			); // REVIEW: Should we be allocating a public table instead of a private one?
			var catchLocals = new SymbolScopeBuilder(
				catchTable,
				null,
				null,
				null,
	...
	export function postAssignScopes(
		ast: AST,
		parent: AST,
		walker: IAstWalker,
	) {
		...
		if (ast) {
			if (ast.nodeType == NodeType.ModuleDeclaration) {
				...
				if (context.modDeclChain.length >= 1) {
					context.typeFlow.checker.currentModDecl =
						context.modDeclChain[context.modDeclChain.length - 1];
				}
			} else if (ast.nodeType == NodeType.ClassDeclaration) {
				popAssignScope(context);
			} else if (ast.nodeType == NodeType.InterfaceDeclaration) {
