...
function f53<T, K extends Exclude<keyof T, symbol>>(
	obj: { [x: string]: boolean },
	k: K,
	s: string,
	n: number,
...
function f83<T extends { [x: string]: { x: any } }, K extends keyof T>(
	obj: T,
	key: K,
) {
...
function f90<T extends S2, K extends keyof S2>(
	x1: S2[keyof S2],
	x2: T[keyof S2],
	x3: S2[K],
) {
...
function path<T, K1 extends keyof T, K2 extends keyof T[K1]>(
	obj: T,
	key1: K1,
	key2: K2,
): T[K1][K2];
...
type Example<T extends { [K in keyof T]: { prop: any } }> = {
...
type DynamicDBRecord<Flag extends string> = (
	...
	DBBoolTable<Flag>;
function getFlagsFromDynamicRecord<Flag extends string>(
...
function ff1<V extends string, T extends string>(
	dd: DictDict<V, T>,
	k1: V,
	k2: T,
): number {
...
function ff2<V extends string, T extends string>(
	dd: DictDict<V, T>,
	k1: V,
	k2: T,
): number {
...
const cf1 = <
	T extends { [P in K]: string } & { cool: string },
	K extends keyof T,
>(
	t: T,
	k: K,
...
const cf2 = <T extends { [P in K | "cool"]: string }, K extends keyof T>(
	t: T,
	k: K,
) => {
	const s: string = t[k];
