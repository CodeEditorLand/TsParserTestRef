...
function f4<T extends { x: string | undefined }>(
	x: T["x"],
	y: NonNullable<T["x"]>,
) {
	x = y;
...
declare function f5<T extends Options, K extends string>(
	p: K,
): Extract<T, { k: K }>;
...
function zeroOf<T extends number | string | boolean>(value: T) {
	return <ZeroOf<T>>(
		(typeof value === "number" ? 0 : typeof value === "string" ? "" : false)
	);
}

function f20<T extends string>(
	n: number,
	b: boolean,
	x: number | boolean,
	y: T,
...
type T36<T> = T extends { a: string }
	? T extends { b: number }
		? T35<T>
		: never
	: never;
type T37<T> = T extends { b: number }
	? T extends { a: string }
		? T35<T>
		: never
	: never;
type T38<T> = [T] extends [{ a: string }]
	? [T] extends [{ b: number }]
		? T35<T>
		: never
	: never;
...
function f50() {
	...
	type Omit<T extends object> = {
		[P in keyof T]: If<Eq<T[P], never>, never, P>;
	}[keyof T];
	type Omit2<T extends object, U = never> = {
		[P in keyof T]: If<Eq<T[P], U>, never, P>;
	}[keyof T];
...
type OldDiff<T extends keyof any, U extends keyof any> = ({ [P in T]: P } & {
	[P in U]: never;
} & { [x: string]: never })[T];
...
type Extracted<Struct> = {
	[K in keyof Struct]: Struct[K] extends FooBar
		? ExtractFooBar<Struct[K]>
		: Struct[K];
};
...
type Weird1 =
	(<U extends boolean>(a: U) => never) extends <U extends true>(a: U) => never
		? never
		: never;

type Weird2 =
	(<U extends boolean>(a: U) => U) extends <U extends true>(a: U) => infer T
		? T
		: never;
