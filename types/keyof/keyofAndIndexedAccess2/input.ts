...
function f2<T extends { [key: string]: number }>(
	a: { x: number; y: number },
	b: { [key: string]: number },
	c: T,
	k: keyof T,
...
function f3b<K extends string>(
	a: { [P in K]: number },
	b: { [P in string]: number },
	k: K,
) {
	a = b; // Error, index signature doesn't imply properties are present
	b = a;
...
function f10<T extends Item, K extends keyof T>(
	obj: T,
	k1: string,
	k2: keyof Item,
	k3: keyof T,
...
function f12<T extends Readonly<Dict>, K extends keyof T>(
	obj: T,
	k1: keyof Dict,
	k2: keyof T,
	k3: K,
...
function fn<
	T extends { elements: Array<string> } | { elements: Array<number> },
>(param: T, cb: (element: T["elements"][number]) => void) {
	cb(param.elements[0]);
}

function fn2<T extends Array<string>>(
	param: T,
...
function fn3<T extends ReadonlyArray<string>>(
	param: T,
	cb: (element: T[number]) => void,
) {
	cb(param[0]);
