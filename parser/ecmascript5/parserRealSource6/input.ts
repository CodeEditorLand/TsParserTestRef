...
module TypeScript {
	...
	export class EnclosingScopeContext {
		...
		public getScriptFragment(): Script {
			if (this.scriptFragment == null) {
				...
				this.scriptFragment = TypeScript.quickParse(
					this.logger,
					ast,
					this.text,
					minChar,
	...
	export function preFindMemberScope(
		ast: AST,
		parent: AST,
		walker: IAstWalker,
	) {
		...
		if (
			hasFlag(ast.flags, memScope.matchFlag) &&
			(memScope.pos < 0 || memScope.pos == ast.limChar)
		) {
			memScope.ast = ast;
	...
	export function pushTypeCollectionScope(
		container: Symbol,
		...
		var builder = new SymbolScopeBuilder(
			valueMembers,
			ambientValueMembers,
			enclosedTypes,
		...
		var chain: ScopeChain = new ScopeChain(
			container,
			context.scopeChain,
			builder,
		);
	...
	export function preFindEnclosingScope(
		ast: AST,
		parent: AST,
		walker: IAstWalker,
	) {
				...
				case NodeType.ClassDeclaration:
					context.scopeGetter = function () {
						return ast.type === null ||
							ast.type.instanceType.containedScope === null
							? null
							: ast.type.instanceType.containedScope;
					};
						...
						if (context.skipNextFuncDeclForClass) {
							...
							context.scopeGetter = function () {
								...
								if (
									...
									hasFlag(
										funcDecl.fncFlags,
										FncFlags.ClassMethod,
									)
								) {
									if (ast.type && ast.type.enclosingType) {
										return ast.type.enclosingType
											.constructorScope;
									}
								}
	...
	//
	export function findEnclosingScopeAt(
		logger: ILogger,
		script: Script,
		text: ISourceText,
		pos: number,
		...
		var context = new EnclosingScopeContext(
			logger,
			script,
			text,
			pos,
		...
		TypeScript.getAstWalkerFactory().walk(
			script,
