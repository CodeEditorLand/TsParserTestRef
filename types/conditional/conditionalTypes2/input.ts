...
class Vector<T> implements Seq<T> {
	...
	partition2<U extends T>(
		predicate: (v: T) => v is U,
	): [Vector<U>, Vector<Exclude<T, U>>];
...
type A<T, V, E> = T extends object
	? {
			[Q in { [P in keyof T]: T[P] extends V ? P : P }[keyof T]]: A<
				T[Q],
				V,
				E
			>;
...
type A2<T, V, E> = T extends object
	? T extends any[]
		? T
		: { [Q in keyof T]: A2<T[Q], V, E> }
	: T;

type B2<T, V> = T extends object
	? T extends any[]
		? T
		: { [Q in keyof T]: B2<T[Q], V> }
	: T;

type C2<T, V, E> = T extends object ? { [Q in keyof T]: C2<T[Q], V, E> } : T;
...
declare function exportCommand<TResponse>(
...
type IRootResponse<TResponse> = TResponse extends IRecord
	? IRecordResponse<TResponse>
	: IResponse<TResponse>;
...
declare type GetPropertyNamesOfType<T, RestrictToType> = {
	[PropertyName in Extract<
		keyof T,
		string
	>]: T[PropertyName] extends RestrictToType ? PropertyName : never;
