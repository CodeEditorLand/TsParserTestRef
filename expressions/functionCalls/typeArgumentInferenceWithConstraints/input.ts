...
function someGenerics2b<T extends string, U extends number>(
	n: (x: T, y: U) => void,
) {}
...
function someGenerics6<A extends number>(
	a: (a: A) => A,
	b: (b: A) => A,
	c: (c: A) => A,
) {}
...
function someGenerics7<A, B extends string, C>(
	a: (a: A) => A,
	b: (b: B) => B,
	c: (c: C) => C,
) {}
...
someGenerics7<number, string, number>(
	(n) => n,
	(n) => n,
	(n) => n,
);
...
function someGenerics8<T extends string>(n: T): T {
	return n;
}
