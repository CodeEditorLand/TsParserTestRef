...
module Formatting {
	export class Indenter implements ILineIndenationResolver {
		...
		constructor(
			...
			this.ApplyScriptBlockIndentation(
				this.languageHostIndentation,
				this.tree,
			);
			this.FillInheritedIndentation(this.tree);
		}

		public GetIndentationEdits(
			token: TokenSpan,
			nextToken: TokenSpan,
			node: ParseNode,
			...
			if (this.logger.information()) {
				this.logger.log(
					"GetIndentationEdits(" +
						...
						"," +
						token.Span.endPosition() +
						"], " +
						"t2=[" +
						(nextToken == null
							? "null"
							: nextToken.Span.startPosition() +
								"," +
								nextToken.Span.endPosition()) +
						"]" +
						")",
				);
			}

			var result = this.GetIndentationEditsWorker(
				token,
				nextToken,
				node,
				sameLineIndent,
				...
				for (var i = 0; i < result.count(); i++) {
					...
					this.logger.log(
						"edit: minChar=" +
							...
							', text="' +
							TypeScript.stringToLiteral(edit.replaceWith, 30) +
							'"',
					);
				}
			}
		...
		public GetIndentationEditsWorker(
			token: TokenSpan,
			nextToken: TokenSpan,
			node: ParseNode,
			sameLineIndent: boolean,
			...
			if (indentationInfo == null) {
				...
				while (
					...
					token.Span.span.start() ==
						node.Parent.AuthorNode.Details.StartOffset
				)
					node = node.Parent;
				...
				if (
					node.CanIndent() &&
					token.Span.span.start() ==
						node.AuthorNode.Details.StartOffset
				) {
					...
					if (
						token.Token == AuthorTokenKind.atkIdentifier &&
						nextToken != null &&
						nextToken.Token == AuthorTokenKind.atkColon
					) {
						...
						indentationInfo = this.ApplyIndentationDeltaFromParent(
							token,
							node,
						);
					}
			...
			if (indentationInfo != null) {
				...
				if (edit != null) {
					...
					if (token.Token == AuthorTokenKind.atkComment) {
						var commentEdits =
							this.GetCommentIndentationEdits(token);
		...
		private GetCommentIndentationEdits(
			token: TokenSpan,
		): List_TextEditInfo {
			var result = new List_TextEditInfo();
			...
			var commentLastLineNumber = this.snapshot.GetLineNumberFromPosition(
				token.Span.endPosition(),
			);
			if (token.lineNumber() == commentLastLineNumber) return result;

			var commentFirstLineIndentationDelta = this.GetIndentationDelta(
				token.Span.startPosition(),
				null,
			);
			if (commentFirstLineIndentationDelta != undefined) {
				for (
					var line = token.lineNumber() + 1;
					line <= commentLastLineNumber;
					line++
				) {
					var lineStartPosition = this.snapshot
						.GetLineFromLineNumber(line)
						.startPosition();
					var lineIndent =
						this.GetLineIndentationForOffset(lineStartPosition);

					var commentIndentationInfo = this.ApplyIndentationDelta2(
						lineIndent,
						commentFirstLineIndentationDelta,
					);
					if (commentIndentationInfo != null) {
						var tokenStartPosition =
							lineStartPosition + lineIndent.length;
						var commentIndentationEdit = this.GetIndentEdit(
							commentIndentationInfo,
							tokenStartPosition,
							false,
						);
		...
		static GetIndentSizeFromIndentText(
			indentText: string,
			editorOptions: Services.EditorOptions,
		): number {
			return GetIndentSizeFromText(
				indentText,
				editorOptions,
				/*includeNonIndentChars:*/ false,
			);
		}

		static GetIndentSizeFromText(
			text: string,
			editorOptions: Services.EditorOptions,
			...
			for (var i = 0; i < text.length; i++) {
				...
				if (c == "\t")
					indentSize =
						...
						(indentSize % editorOptions.TabSize);
		...
		private GetSpecialCaseIndentation(
			token: TokenSpan,
			node: ParseNode,
		): IndentationInfo {
			var indentationInfo: IndentationInfo = null;
			...
			switch (token.Token) {
				case AuthorTokenKind.atkLCurly: // { is not part of the tree
					indentationInfo =
						this.GetSpecialCaseIndentationForLCurly(node);
				...
				case AuthorTokenKind.atkRCurly: // } is not part of the tree
					...
					if (
						node.AuthorNode.Details.Kind ==
							AuthorParseNodeKind.apnkBlock &&
						node.AuthorNode.EdgeLabel ==
							AuthorParseNodeEdge.apneBody
					)
						node = node.Parent;
					indentationInfo = node.GetNodeStartLineIndentation(this);
					return indentationInfo;
				...
				case AuthorTokenKind.atkWhile: // while (in do-while) is not part of the tree
					if (
						node.AuthorNode.Details.Kind ==
						AuthorParseNodeKind.apnkDoWhile
					) {
						indentationInfo =
							node.GetNodeStartLineIndentation(this);
						return indentationInfo;
					}
				...
				case AuthorTokenKind.atkSColon:
					return this.GetSpecialCaseIndentationForSemicolon(
						token,
						node,
					);

				case AuthorTokenKind.atkComment:
					return this.GetSpecialCaseIndentationForComment(
						token,
						node,
					);
		...
		private GetSpecialCaseIndentationForLCurly(
			node: ParseNode,
		): IndentationInfo {
			var indentationInfo: IndentationInfo = null;
			...
			if (
				node.AuthorNode.Details.Kind ==
					AuthorParseNodeKind.apnkFncDecl ||
				node.AuthorNode.EdgeLabel == AuthorParseNodeEdge.apneThen ||
				node.AuthorNode.EdgeLabel == AuthorParseNodeEdge.apneElse
			) {
				...
				node.AuthorNode.Details.Kind ==
					AuthorParseNodeKind.apnkObject &&
				!node.CanIndent()
			) {
		...
		private GetSpecialCaseIndentationForSemicolon(
			token: TokenSpan,
			node: ParseNode,
		): IndentationInfo {
			var indentationInfo: IndentationInfo = null;
			...
			if (this.smartIndent) {
				...
				if (
					node.AuthorNode.Details.Kind != AuthorParseNodeKind.apnkFor
				) {
					...
					var semiColonStartSpan = new Span(
						token.Span.startPosition(),
						0,
					);
					node = ParseTree.FindCommonParentNode(
						semiColonStartSpan,
						semiColonStartSpan,
						node,
					);
		...
		private GetSpecialCaseIndentationForComment(
			token: TokenSpan,
			node: ParseNode,
		): IndentationInfo {
			var indentationInfo: IndentationInfo = null;
			...
			var twoCharSpan = token.Span.Intersection(
				new Span(token.Span.startPosition(), 2),
			);
			if (
				twoCharSpan != null &&
				(twoCharSpan.GetText() == "//" || twoCharSpan.GetText() == "/*")
			) {
				while (
				...
				if (this.CanIndentComment(token, node)) {
					indentationInfo =
						node.GetEffectiveChildrenIndentationForComment(this);
				} else {
					indentationInfo = this.ApplyIndentationDeltaFromParent(
						token,
						node,
		...
		private CanIndentComment(token: TokenSpan, node: ParseNode): boolean {
			switch (node.AuthorNode.Details.Kind) {
				...
				case AuthorParseNodeKind.apnkFncDecl:
					...
					//              /* test */ b)
					var result = true;
					var children = ParseNodeExtensions.FindChildrenWithEdge(
						node,
						AuthorParseNodeEdge.apneArgument,
					);
					children.foreach((argumentNode) => {
						if (
							token.Span.startPosition() <
							argumentNode.AuthorNode.Details.StartOffset
						)
							result = false;
					});
		...
		private ApplyScriptBlockIndentation(
			...
			var scriptBlockIndentation = this.ApplyIndentationLevel(
		...
		private GetIndentEdit(
			indentInfo: IndentationInfo,
			tokenStartPosition: number,
			sameLineIndent: boolean,
		): TextEditInfo {
			var indentText = this.ApplyIndentationLevel(
				indentInfo.Prefix,
				indentInfo.Level,
			);
			...
			if (sameLineIndent) {
				...
				var snapshotLine =
					this.snapshot.GetLineFromPosition(tokenStartPosition);
				var currentIndentSpan = new Span(
					snapshotLine.startPosition(),
					tokenStartPosition - snapshotLine.startPosition(),
				);
				var currentIndentText =
					this.snapshot.GetText(currentIndentSpan);

				if (currentIndentText !== indentText) {
					if (this.logger.debug()) {
						// Verify that currentIndentText is all whitespaces
						for (
							var i = 0, len = currentIndentText.length;
							i < len;
							i++
						) {
							...
							if (!StringUtils.IsWhiteSpace(c)) {
								Debug.Fail(
									"Formatting error: Will remove user code when indenting the line: " +
										snapshotLine.getText(),
								);
								break;
							}
						}
					...
					return new TextEditInfo(
						currentIndentSpan.start(),
						currentIndentSpan.length(),
						indentText,
					);
				}
			}
		...
		private GetIndentString(
			...
			var tabString = convertTabsToSpaces
				? StringUtils.create(" ", tabSize)
				: "\t";
		...
		private ApplyIndentationDeltaFromParent(
			token: TokenSpan,
			node: ParseNode,
		): IndentationInfo {
			var indentationInfo: IndentationInfo = null;
			...
			if (
				indentableParent != null &&
				indentableParent.AuthorNode.Details.Kind !=
					AuthorParseNodeKind.apnkProg
			) {
				var parentIndentationDeltaSize = this.GetIndentationDelta(
					indentableParent.AuthorNode.Details.StartOffset,
					token.Span.startPosition(),
				);
				if (parentIndentationDeltaSize !== undefined) {
					indentationInfo = this.ApplyIndentationDelta1(
						token.Span.startPosition(),
						parentIndentationDeltaSize,
					);
				}
			}
		...
		private ApplyIndentationDelta1(
			tokenStartPosition: number,
			delta: number,
		): IndentationInfo {
		...
		private ApplyIndentationDelta2(
			currentIndent: string,
			delta: number,
		): IndentationInfo {
			if (delta == 0) return null;

			var currentIndentSize = Indenter.GetIndentSizeFromIndentText(
				currentIndent,
				this.editorOptions,
			);
			...
			var newIndent = this.GetIndentString(
				null,
				newIndentSize,
				this.editorOptions.TabSize,
				this.editorOptions.ConvertTabsToSpaces,
			);
			if (newIndent != null) {
		...
		private GetIndentationDelta(
			tokenStartPosition: number,
			childTokenStartPosition: number /*?*/,
		): number /*?*/ {
			Debug.Assert(
				childTokenStartPosition !== undefined,
				"Error: caller must pass 'null' for undefined position",
			);

			var indentationDeltaSize =
				this.offsetIndentationDeltas.GetValue(tokenStartPosition);
			if (indentationDeltaSize === null) {
				var indentEditInfo =
					this.indentationBag.FindIndent(tokenStartPosition);

				// No recorded indentation, return null
				if (indentEditInfo == null) return null;

				var origIndentText = this.snapshot.GetText(
					new Span(
						indentEditInfo.OrigIndentPosition,
						indentEditInfo.OrigIndentLength(),
					),
				);
				var newIndentText = indentEditInfo.Indentation();

				var origIndentSize = Indenter.GetIndentSizeFromText(
					origIndentText,
					this.editorOptions,
					/*includeNonIndentChars*/ true,
				);
				var newIndentSize = Indenter.GetIndentSizeFromIndentText(
					newIndentText,
					this.editorOptions,
				);
				...
				if (childTokenStartPosition !== null) {
					var childTokenLineStartPosition = this.snapshot
						.GetLineFromPosition(childTokenStartPosition)
						.startPosition();
					var childIndentText = this.snapshot.GetText(
						new Span(
							childTokenLineStartPosition,
							childTokenStartPosition -
								childTokenLineStartPosition,
						),
					);

					var childIndentSize = Indenter.GetIndentSizeFromIndentText(
						childIndentText,
						this.editorOptions,
					);

					if (childIndentSize < origIndentSize)
						origIndentSize = Indenter.GetIndentSizeFromIndentText(
							origIndentText,
							this.editorOptions,
						);
				}
				...
				this.offsetIndentationDeltas.Add(
					tokenStartPosition,
		...
		private FillInheritedIndentation(tree: ParseTree): void {
			...
			if (tree.StartNodeSelf != null) {
				if (
					!this.smartIndent &&
					tree.StartNodePreviousSibling !== null &&
					tree.StartNodeSelf.AuthorNode.Label == 0 &&
					tree.StartNodePreviousSibling.Label == 0
				) {
					...
					var lineNum =
						this.snapshot.GetLineNumberFromPosition(offset);
					var node = indentNode;
					while (
						node.Parent != null &&
						this.snapshot.GetLineNumberFromPosition(
							node.Parent.AuthorNode.Details.StartOffset,
						) == lineNum
					) {
						node = node.Parent;
					...
					if (this.smartIndent) {
						...
						while (
							parent != null &&
							parent.AuthorNode.Details.StartOffset ==
								this.firstToken.Span.startPosition()
						)
							parent = parent.Parent;
					} else {
						// Get the parent that is really on a different line from the self node
						var startNodeLineNumber =
							this.snapshot.GetLineNumberFromPosition(
								tree.StartNodeSelf.AuthorNode.Details
									.StartOffset,
							);
					...
					if (
						parent != null &&
						parent.AuthorNode.Details.Kind !=
							AuthorParseNodeKind.apnkProg
					) {
			...
			if (indentNode != null) {
				...
				if (
					...
					ParseNodeExtensions.GetChildren(indentNode.Parent).foreach(
						(sibling) => {
							if (sibling !== indentNode) {
								if (sibling.CanIndent())
									sibling.SetIndentationOverride(
										indentOverride,
									);
							}
						},
				...
				var lastDelta = 0;
				var lastLine = this.snapshot.GetLineNumberFromPosition(
					indentNode.AuthorNode.Details.StartOffset,
				);
				do {
					var currentLine = this.snapshot.GetLineNumberFromPosition(
						indentNode.AuthorNode.Details.StartOffset,
					);
					if (lastLine != currentLine) {
						lastLine = currentLine;
						indentOverride = this.ApplyIndentationLevel(
							indentOverride,
							-lastDelta,
						);
						lastDelta = 0;
		...
		private RegisterIndentation(
			...
			if (sameLineIndent) {
				...
				var lineStartPosition = this.snapshot
					.GetLineFromPosition(indent.Position)
					.startPosition();
				var lineIndentLength = indent.Position - lineStartPosition;

				indentationInfo = IndentationEditInfo.create2(
					indent.Position,
					indent.ReplaceWith,
					lineStartPosition,
					lineIndentLength,
		...
		public RegisterIndentation2(position: number, indent: string): void {
			this.RegisterIndentation(
				new TextEditInfo(position, 0, indent),
				false,
			);
		}

		private AdjustStartOffsetIfNeeded(
			token: TokenSpan,
			node: ParseNode,
		): void {
			...
			switch (token.Token) {
				...
				case AuthorTokenKind.atkLBrack:
					updateStartOffset =
						node.AuthorNode.Details.Kind ==
						AuthorParseNodeKind.apnkArray;
					break;
			}

			if (updateStartOffset) {
				ParseNodeExtensions.SetNodeSpan(
					node,
					token.Span.startPosition(),
					node.AuthorNode.Details.EndOffset,
				);
			}
		}
		...
		private IsMultiLineString(token: TokenSpan): boolean {
			return (
				token.tokenID === TypeScript.TokenID.StringLiteral &&
				this.snapshot.GetLineNumberFromPosition(
					token.Span.endPosition(),
				) >
					this.snapshot.GetLineNumberFromPosition(
